requirejs(['serializeObject', 'art-template', 'moment'], function (_, template, moment) {
    ...
    function listData() {
        $('#dg_depart').datagrid({
            ...
            url: ctx + "/Depart/getDepart",
            columns: [[
                {
                    field: 'name', title: '部门名称', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                {
                    field: 'code', title: '部门编码', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.result == true) {
                            ...
                            $('#dg_depart').datagrid('reload', searchData);
                        } else {
            ...
            onClickRow: function (rowIndex, rowData) {
                ...
                if (editRow != undefined) {
                    $('#dg_depart').datagrid('unselectRow', rowIndex);
                }
            }
        });
    }
    ...
    $("#addButton").click(function () {
        if ($('#addButton').linkbutton('options').disabled == false) {
            ...
            if (editRow == undefined) {
                $('#dg_depart').datagrid("insertRow", {
                    index: 0,
                    row: {}
                });
                $('#dg_depart').datagrid("unselectAll");
                editRow = 0;
                $('#dg_depart').datagrid("beginEdit", editRow);
            }
        }
    });
    ...
    $("#editButton").click(function () {
        if ($('#editButton').linkbutton('options').disabled == false) {
            //修改时要获取选择到的行
            var rows = $('#dg_depart').datagrid("getSelections");
            //如果只选择了一行则可以进行修改，否则不操作
            if (rows.length == 1) {
                setButtonState('disable', 'disable', 'disable', 'disable', 'enable', 'disable', 'enable');
                //修改之前先关闭已经开启的编辑行，当调用endEdit该方法时会触发onAfterEdit事件
                if (editRow != undefined) {
                    $('#dg_depart').datagrid("endEdit", editRow);
                }
                //当无编辑行时
                if (editRow == undefined) {
                    //获取到当前选择行的下标
                    var index = $('#dg_depart').datagrid("getRowIndex", rows[0]);
                    //开启编辑
                    $('#dg_depart').datagrid("beginEdit", index);
    ...
    $('#redoButton').click(function () {
        if ($('#redoButton').linkbutton('options').disabled == false) {
            ...
            editRow = undefined;
            $('#dg_depart').datagrid("rejectChanges");
            $('#dg_depart').datagrid("unselectAll");
            setButtonState('enable', 'enable', 'enable', 'enable', 'disable', 'enable', 'disable');
        }
    });
