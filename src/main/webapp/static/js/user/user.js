requirejs(['serializeObject', 'art-template', 'moment'], function (_, template, moment) {
    ...
    function listData() {
        $('#dg_user').datagrid({
            ...
            columns: [[
                {
                    field: 'code', title: '编号', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                {
                    field: 'name', title: '姓名', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                {
                    field: 'pwd', title: '密码', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                {
                    field: 'departId', title: '部门', width: fixWidth(0.30),
                    editor: {
                        type: 'combobox',
                        options: {
                            valueField: 'name',
                            textField: 'name',
                            method: 'post',
                            url: ctx + "/Depart/getDepartRow"
                        }
                    }
                },
                {
                    field: 'powerStatus', title: '职位', width: fixWidth(0.30),
                    editor: {
                        type: 'combobox',
                        options: {
                            valueField: 'post',
                            textField: 'post',
                            method: 'post',
                            url: ctx + "/position/getPositionRow"
                        }
                    }
                }
            ]],
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.result == true) {
                            ...
                            $('#dg_user').datagrid('reload', searchData);
                        } else {
            ...
            onClickRow: function (rowIndex, rowData) {
                ...
                if (editRow != undefined) {
                    $('#dg_user').datagrid('unselectRow', rowIndex);
                }
            }
        });
    }
    ...
    $("#addButton").click(function () {
        if ($('#addButton').linkbutton('options').disabled == false) {
            ...
            if (editRow == undefined) {
                ...
                $('#dg_user').datagrid("unselectAll");
                editRow = 0;
                $('#dg_user').datagrid("beginEdit", editRow);
            }
        }
    });
    ...
    $("#editButton").click(function () {
        if ($('#editButton').linkbutton('options').disabled == false) {
            ...
            if (rows.length == 1) {
                ...
                if (editRow == undefined) {
                    //获取到当前选择行的下标
                    var index = $('#dg_user').datagrid("getRowIndex", rows[0]);
                    //开启编辑
                    $('#dg_user').datagrid("beginEdit", index);
    ...
    $('#redoButton').click(function () {
        if ($('#redoButton').linkbutton('options').disabled == false) {
            ...
            editRow = undefined;
            $('#dg_user').datagrid("rejectChanges");
            $('#dg_user').datagrid("unselectAll");
            setButtonState('enable', 'enable', 'enable', 'enable', 'disable', 'enable', 'disable');
        }
    });
