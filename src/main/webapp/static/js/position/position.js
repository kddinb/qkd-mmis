requirejs(['serializeObject', 'art-template', 'moment'], function (_, template, moment) {
    ...
    function listData() {
        $('#dg_position').datagrid({
            ...
            url: ctx + "/position/getPosition",
            columns: [[
                {
                    field: 'post', title: '职位名称', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                {
                    field: 'code', title: '职位代码', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.result == true) {
                            ...
                            $('#dg_position').datagrid('reload', searchData);
                        } else {
            ...
            onClickRow: function (rowIndex, rowData) {
                ...
                if (editRow != undefined) {
                    $('#dg_position').datagrid('unselectRow', rowIndex);
                }
            }
        });
    }
    ...
    $("#searchButton").click(function () {
        if ($('#searchButton').linkbutton('options').disabled == false) {
            ...
            $('#dg_position').datagrid('load', data);
        }
    });
    ...
    $("#addButton").click(function () {
        if ($('#addButton').linkbutton('options').disabled == false) {
            ...
            if (editRow == undefined) {
                $('#dg_position').datagrid("insertRow", {
                    index: 0,
                    row: {}
                });
                $('#dg_position').datagrid("unselectAll");
                editRow = 0;
                $('#dg_position').datagrid("beginEdit", editRow);
            }
        }
    });
    ...
    $("#editButton").click(function () {
        if ($('#editButton').linkbutton('options').disabled == false) {
            //修改时要获取选择到的行
            var rows = $('#dg_position').datagrid("getSelections");
            //如果只选择了一行则可以进行修改，否则不操作
            if (rows.length == 1) {
                setButtonState('disable', 'disable', 'disable', 'disable', 'enable', 'disable', 'enable');
                //修改之前先关闭已经开启的编辑行，当调用endEdit该方法时会触发onAfterEdit事件
                if (editRow != undefined) {
                    $('#dg_position').datagrid("endEdit", editRow);
                }
                //当无编辑行时
                if (editRow == undefined) {
                    //获取到当前选择行的下标
                    var index = $('#dg_position').datagrid("getRowIndex", rows[0]);
                    //开启编辑
                    $('#dg_position').datagrid("beginEdit", index);
    ...
    $('#redoButton').click(function () {
        if ($('#redoButton').linkbutton('options').disabled == false) {
            ...
            editRow = undefined;
            $('#dg_position').datagrid("rejectChanges");
            $('#dg_position').datagrid("unselectAll");
            setButtonState('enable', 'enable', 'enable', 'enable', 'disable', 'enable', 'disable');
        }
    });
            ...
            if (rows.length > 0) {
                $.messager.confirm("提示", "你确定要删除吗?", function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: ctx + "/position/removePosition",
                            data: ids,
                            dataType: "json",
                            success: function (data) {
                                if (data.result == true) {
