requirejs(['serializeObject', 'art-template', 'moment'], function (_, template, moment) {
    ...
    function listData() {
        $('#dg_supplier').datagrid({
            ...
            url: ctx + "/supplier/getSupplier",
            columns: [[
                {
                    field: 'name', title: '供应商', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                {
                    field: 'code', title: '供应商编码', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                {
                    field: 'contact', title: '联系人', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                {
                    field: 'telephone', title: '联系电话', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                {
                    field: 'addr', title: '联系地址', width: fixWidth(0.40),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                {
                    field: 'bank', title: '开户银行', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                }, {
                    field: 'bankAccount', title: '银行账号', width: fixWidth(0.30),
                    editor: {
                        type: 'text',
                        options: {}
                    }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.result == true) {
                            ...
                            $('#dg_supplier').datagrid('reload', searchData);
                        } else {
            ...
            onClickRow: function (rowIndex, rowData) {
                ...
                if (editRow != undefined) {
                    $('#dg_supplier').datagrid('unselectRow', rowIndex);
                }
            }
        });
    }
    ...
    $("#addButton").click(function () {
        if ($('#addButton').linkbutton('options').disabled == false) {
            ...
            if (editRow == undefined) {
                $('#dg_supplier').datagrid("insertRow", {
                    index: 0,
                    row: {}
                });
                $('#dg_supplier').datagrid("unselectAll");
                editRow = 0;
                $('#dg_supplier').datagrid("beginEdit", editRow);
            }
        }
    });
    ...
    $("#editButton").click(function () {
        if ($('#editButton').linkbutton('options').disabled == false) {
            //修改时要获取选择到的行
            var rows = $('#dg_supplier').datagrid("getSelections");
            //如果只选择了一行则可以进行修改，否则不操作
            if (rows.length == 1) {
                setButtonState('disable', 'disable', 'disable', 'disable', 'enable', 'disable', 'enable');
                //修改之前先关闭已经开启的编辑行，当调用endEdit该方法时会触发onAfterEdit事件
                if (editRow != undefined) {
                    $('#dg_supplier').datagrid("endEdit", editRow);
                }
                //当无编辑行时
                if (editRow == undefined) {
                    //获取到当前选择行的下标
                    var index = $('#dg_supplier').datagrid("getRowIndex", rows[0]);
                    //开启编辑
                    $('#dg_supplier').datagrid("beginEdit", index);
    ...
    $('#redoButton').click(function () {
        if ($('#redoButton').linkbutton('options').disabled == false) {
            ...
            editRow = undefined;
            $('#dg_supplier').datagrid("rejectChanges");
            $('#dg_supplier').datagrid("unselectAll");
            setButtonState('enable', 'enable', 'enable', 'enable', 'disable', 'enable', 'disable');
        }
    });
            ...
            if (rows.length > 0) {
                $.messager.confirm("提示", "你确定要删除吗?", function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: ctx + "/supplier/removeSupplier",
                            data: ids,
                            dataType: "json",
                            success: function (data) {
                                if (data.result == true) {
